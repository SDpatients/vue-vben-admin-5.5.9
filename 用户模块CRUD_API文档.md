...
- **Base URL**: `http://192.168.0.151:8080`
- **API前缀**: `/api/users`
...
spring:
  datasource:
    url: jdbc:mysql://192.168.0.151:3306/law_db?useSSL=false&serverTimezone=UTC
    username: root
...
```
http://192.168.0.151:8080/swagger-ui.html
```
...
```bash
curl -X POST http://192.168.0.151:8080/api/users \
...
```bash
curl -X GET "http://192.168.0.151:8080/api/users?page=1&size=10&status=ACTIVE" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -X GET http://192.168.0.151:8080/api/users/1 \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -X PUT http://192.168.0.151:8080/api/users/1 \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "realName": "更新后的用户",
...
```bash
curl -X DELETE http://192.168.0.151:8080/api/users/1 \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
