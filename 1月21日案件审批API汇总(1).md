...
```json
{
  ...
  "approvalTitle": "案件提交审批",    // 审核标题（必填）
  "approvalContent": "详细的审批内容",  // 审核内容
  "approvalAttachment": "file1.pdf,file2.pdf",  // 审核附件，多个附件用逗号分隔
  "remark": "请尽快审核"
}
```
...
```json
{
  "approvalTitle": "更新后的审核标题",
  "approvalContent": "更新后的审批内容",
  "approvalAttachment": "updated_file.pdf",
  "remark": "更新后的备注"
}
```
...
```javascript
// 请求
fetch('http://192.168.0.151:8080/approval', {
  method: 'POST',
  headers: {
...
```javascript
// 请求
fetch('http://192.168.0.151:8080/approval/1/approve', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer token'
