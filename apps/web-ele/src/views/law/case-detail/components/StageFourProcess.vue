...
const progress = computed(() => {
  ...
  const completedTasks = tasks.value.filter(
    (task) => (task.status === '完成' || task.status === '跳过') && !task.isPending,
  ).length;
...
const loadTaskData = async () => {
  ...
  try {
    ...
    // 处理API响应
    const getStatus = (res: PromiseSettledResult<any>): CaseProcessApi.TaskStatus => {
      if (res.status === 'fulfilled' && res.value && res.value.status === '1' && res.value.data?.records?.length > 0) {
        return (res.value.data.records[0].DQZT as CaseProcessApi.TaskStatus) || '未确认';
      }
      return '未确认';
    };
    ...
    tasks.value = [
      ...
      },
      {
        id: 'meetingDocuments',
        name: '会议文件',
        status: getStatus(meetingDocumentsRes),
        apiUrl: '/api/web/getAllMeetingDocuments',
        token: 'd7e31d46d165d1040ebc2537db8ea110',
      },
      {
      ...
      },
      {
        id: 'importantActions',
        name: '重要行为',
        status: getStatus(importantActionsRes),
        apiUrl: '/api/web/getAllImportantActions',
        token: '105e75ba651c5723542fab64984d8f52',
      },
      {
        id: 'setoffReview',
        name: '抵销审核',
        status: getStatus(setoffReviewRes),
        apiUrl: '/api/web/getAllSetoffReview',
        token: 'a0995016aa156d8efc752b7f3895561e',
      },
      {
        id: 'auditReport',
        name: '审计报告',
        status: getStatus(auditReportRes),
        apiUrl: '/api/web/getAllAuditReport',
        token: '41b31a3d701e888e11b9d2470b0b2ad9',
      },
    ];
        ...
        >
          ...
          <div class="task-actions">
            <ElButton
              ...
              :disabled="task.status === '完成' || task.status === '跳过' || task.isPending"
            >
