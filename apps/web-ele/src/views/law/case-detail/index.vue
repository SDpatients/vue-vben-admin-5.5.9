...
const claimRegistrationTabsRef = ref<InstanceType<typeof ClaimRegistrationTabs> | null>(null);

// 进度管理弹窗相关
const showProgressManagement = ref(false);

// 处理导航到债权登记表模块
const handleNavigateToClaimRegistration = () => {
  activeTab.value = 'claimRegistration';
  nextTick(() => {
    claimRegistrationTabsRef.value?.handleAdd();
  });
};
    ...
    modules: [
      {
        code: 'TASK_012',
        name: '会议资料',
        description: '管理人筹备与统计债权人会议。需上传：1.会议议程；2.会议材料（债权表、管理人工作报告、财产管理方案等）；3.会议通知及送达证明；4.会场布置及设备准备情况。需确认：1.会议材料是否充分完整；2.通知是否送达所有债权人；3.筹备工作是否符合法律规定。'
      },
      {
        code: 'TASK_013',
        name: '表决事项和表决结果',
...
const isViewingWorkLog = ref(false);
    ...
    if (isEditingWorkLog.value && currentWorkLogId.value) {
      ...
      if (response.code === 200) {
        ...
        if (allUploadedFileIds.length > 0) {
          // 合并原有的 attachmentIds 和新上传的文件 ID
          const existingAttachmentIds = workLogForm.attachmentIds 
            ? workLogForm.attachmentIds.split(',').filter(id => id.trim()) 
            : [];
...
const submitDirectUploadForm = async () => {
  ...
  try {
    ...
    if (documentUploadRef.value) {
      ...
      console.log('documentUploadRef.value 是 undefined');
    }
  ...
  try {
    ...
    if (response.code === 200 && response.data) {
      ...
      let list = (response.data.list || []).map((item: any) => ({
        ...
        date: item.publishTime, // 将 publishTime 映射到 date 字段用于表格显示
      }));
...
const saveAnnouncement = async () => {
  ...
  try {
    ...
    const requestData = {
      ...
      publishTime: announcementData.publishTime || undefined,
    };

    console.log('=== 保存公告 ===');
    console.log('isEditingAnnouncement:', isEditingAnnouncement.value);
    ...
    if (isEditingAnnouncement.value && currentAnnouncementId.value) {
      ...
      if (response.code === 200) {
        ...
        console.log('公告更新成功，ID:', announcementId);
      ...
      if (response.code === 200) {
        ...
        console.log('公告创建成功，ID:', announcementId);
    ...
    if (!announcementId) {
      console.error('公告 ID 为空，无法上传文件');
      ElMessage.error('公告保存成功，但返回的 ID 为空');
      return;
    }
    ...
    if (announcementUploadRef.value && announcementUploadRef.value.getHasUntransferredFiles()) {
      console.log('发现手机上传的临时文件，开始转移...');
      const transferredFiles = await announcementUploadRef.value.transferMobileFiles(announcementId);
    ...
    // 2. 上传本地文件（电脑选择的文件）
    console.log('=== 准备公告文件上传 ===');
    console.log('announcementData.attachments:', announcementData.attachments);
    console.log('announcementUploadRef.value:', announcementUploadRef.value);
    console.log('announcementUploadRef.value.getLocalFiles():', announcementUploadRef.value?.getLocalFiles());
    ...
    if (announcementData.attachments && announcementData.attachments.length > 0) {
      ...
      if (filesToUpload.length > 0) {
        try {
          ...
          if (uploadResponse.code === 200) {
            ...
            if (uploadResponse.data && Array.isArray(uploadResponse.data)) {
              const uploadedIds = uploadResponse.data.map((f: any) => f.id).filter((id: number) => id);
              allUploadedFileIds = [...allUploadedFileIds, ...uploadedIds];
            }
          } else {
        ...
        console.log('没有需要上传的新文件');
      }
    }

    console.log('所有上传的文件 ID:', allUploadedFileIds);
...
const openNewAnnouncementDialog = async () => {
  ...
  announcementData.publishTime = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`;
      ...
      >
        ...
        <div v-if="activeTab === 'process'">
          ...
          <BankruptcyProcess
            ...
            @navigate-to-claim-registration="handleNavigateToClaimRegistration"
          />
        </div>
        ...
        <ElDialog
          ...
          :title="isViewingWorkLog ? '查看工作日志' : (isEditingWorkLog ? '编辑工作日志' : '添加工作日志')"
          width="800px"
          destroy-on-close
        >
          <ElForm
          ...
          <template #footer>
            <ElButton @click="showWorkLogDialog = false">
              {{ isViewingWorkLog ? '关闭' : '取消' }}
            </ElButton>
            <ElButton
              v-if="!isViewingWorkLog"
              type="primary"
              @click="saveWorkLog"
          ...
          <div class="dialog-form">
            ...
            <ElFormItem label="附件">
              <FileUpload
                ...
                :disabled="false"
                :local-mode="true"
                @local-files-change="(files) => { console.log('公告附件变化:', files); announcementData.attachments = files; }"
              />
            </ElFormItem>
          </div>
          <template #footer>
