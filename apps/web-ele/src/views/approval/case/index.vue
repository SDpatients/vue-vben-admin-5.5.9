  ...
  <div class="case-approval-page p-6">
    ...
    >
      <div v-if="currentCase" class="case-detail">
        ...
        <template v-else>
          <!-- 任务信息 -->
          <div v-if="contentData && 'task' in contentData && contentData.task" class="task-info-section">
          ...
          <!-- 提交记录列表 -->
          <div v-if="contentData && 'submissions' in contentData && contentData.submissions && contentData.submissions.length > 0" class="submissions-section">
            <div class="section-title">提交记录 ({{ contentData.submissions.length }}条)</div>
            <div class="submissions-list">
              <div v-for="(submission, index) in contentData.submissions" :key="submission.id" class="submission-item">
                <div class="submission-header">
                  <span class="submission-index">提交 {{ index + 1 }}</span>
                  <span class="submission-title">{{ submission.submissionTitle }}</span>
                  <ElTag :type="submission.status === 'APPROVED' ? 'success' : 'warning'" size="small">
                    {{ submission.status === 'APPROVED' ? '已通过' : '待审核' }}
                  </ElTag>
                </div>
                <div class="submission-content">
                  <div class="content-label">提交内容：</div>
                  <div class="content-text">{{ submission.submissionContent }}</div>
                </div>
                <div class="submission-meta">
                  <span>提交时间：{{ formatDateTime(submission.createTime) }}</span>
                </div>
                
                <!-- 该提交的附件 -->
                <div v-if="attachmentData && 'files' in attachmentData && attachmentData.files[submission.id]" class="submission-files">
                  <div class="files-label">附件文件 ({{ attachmentData.files[submission.id].length }}个)：</div>
                  <div class="files-list">
                    <div v-for="file in attachmentData.files[submission.id]" :key="file.id" class="file-item">
                      <div class="file-info">
                        <div class="file-icon">
                          <i v-if="file.originalFileName?.match(/\.(jpg|jpeg|png|gif|bmp)$/i)" class="i-lucide-file-image"></i>
                          <i v-else-if="file.originalFileName?.includes('.pdf')" class="i-lucide-file-text"></i>
                        ...
                        <div class="file-details">
                          ...
                          <div class="file-meta">
                            <span>{{ file.fileSize ? formatFileSize(file.fileSize) : '未知大小' }}</span>
                          </div>
                        </div>
                      </div>
                      <div class="file-actions">
                        ...
                        <ElButton 
                          ...
                          size="small" 
                          link 
                          @click.stop="handleDownloadFile(file.id, file.originalFileName, file.filePath)"
                        >
          ...
          <!-- 无附件提示 -->
          <div v-if="(!attachmentData || !('files' in attachmentData) || Object.keys(attachmentData.files).length === 0) && (!contentData || !('submissions' in contentData) || contentData.submissions.length === 0)" class="no-attachments">
