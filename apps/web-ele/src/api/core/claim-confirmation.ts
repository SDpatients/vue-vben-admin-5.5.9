...
export namespace ClaimConfirmationApi {
  /** 确认状态枚举 */
  export type ConfirmationStatus = 'PENDING' | 'CONFIRMED' | 'OBJECTION' | 'COURT' | 'LAWSUIT';

  /** 表决结果枚举 */
  export type VoteResult = 'AGREE' | 'DISAGREE' | 'ABSTAIN';

  /** 会议类型枚举 */
  export type MeetingType = 'FIRST' | 'SECOND' | 'TEMPORARY';

  /** 裁定结果枚举 */
  export type CourtRulingResult = 'CONFIRMED' | 'PARTIAL_CONFIRMED' | 'UNCONFIRMED';

  /** 诉讼状态枚举 */
  export type LawsuitStatus = 'PENDING' | 'TRIALING' | 'JUDGED' | 'EXECUTING' | 'COMPLETED';

  /** 诉讼结果枚举 */
  export type LawsuitResult = 'WIN' | 'LOSE' | 'PARTIAL' | 'SETTLED';

  /** 最终确认依据枚举 */
  export type FinalConfirmationBasis = 'MEETING' | 'COURT' | 'SETTLEMENT' | 'OTHER';
