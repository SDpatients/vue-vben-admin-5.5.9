<template>
  <view class="form-container">
    <view class="form-header">
      <text class="title">{{ isEdit ? '编辑债权人' : '新增债权人' }}</text>
    </view>
    ...
    <scroll-view class="form-body" scroll-y>
      <view class="form-section">
        ...
        <view class="form-group">
          <text class="label required">债权人名称</text>
          <input
        ...
        <view class="form-group">
          <text class="label required">债权人类型</text>
          <view class="radio-group">
            <view
              v-for="item in creditorTypeOptions"
              :key="item.value"
              :class="['radio-item', { active: formData.creditorType === item.value }]"
              @click="formData.creditorType = item.value"
            >
        ...
        <view class="form-group" v-if="formData.creditorType === '企业'">
          ...
          <input
            v-model.number="formData.registeredCapital"
            type="digit"
            placeholder="请输入注册资本"
            class="form-input"
          />
...
const creditorId = ref<number | null>(null)
const isEdit = computed(() => creditorId.value !== null)
...
const loadDetail = async (id: number) => {
  try {
    ...
    formData.value = {
      ...
      creditorType: data.creditorType || '个人',
...
const validateForm = (): boolean => {
  ...
  if (!formData.value.creditorName.trim()) {
    uni.showToast({ title: '请输入债权人名称', icon: 'none' })
    return false
  }
  if (!formData.value.creditorType) {
    uni.showToast({ title: '请选择债权人类型', icon: 'none' })
    return false
  }
  return true
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value && creditorId.value) {
      await updateCreditor(creditorId.value, { id: creditorId.value, ...submitData })
      uni.showToast({ title: '更新成功', icon: 'success' })
    } else {
      await createCreditor(submitData)
      uni.showToast({ title: '创建成功', icon: 'success' })
    }
    uni.$emit('refresh-creditor-list')
    setTimeout(() => {
      uni.navigateBack()
    }, 1500)
  } catch (error) {
