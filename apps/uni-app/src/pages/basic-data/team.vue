<template>
  <view class="team-container">
    ...
    <view class="table-header">
      <view class="th th-name">团队信息</view>
    ...
    <scroll-view class="table-body" scroll-y @scrolltolower="onLoadMore">
        ...
        <view class="td td-name">
          <view class="name-cell">
            ...
            <view class="name-info">
              ...
              <text class="desc" v-if="item.teamDescription">{{ item.teamDescription }}</text>
            </view>
          </view>
        </view>
      ...
      <view class="no-more" v-else-if="!hasMore && teamList.length > 0">
        <text>没有更多了</text>
      </view>
      <view class="empty" v-else-if="teamList.length === 0">
...
const teamList = shallowRef<WorkTeamItem[]>([])
const page = ref(1)
const pageSize = 10
const total = ref(0)
...
const loadData = async (isRefresh = false) => {
  ...
  try {
    ...
    if (searchKeyword.value.trim()) {
      ...
      params.teamName = searchKeyword.value.trim()
    }
    if (filterParams.value.status) {
      params.status = filterParams.value.status
    }

    const res = await getWorkTeamList(params)
    const listData = res.data?.list || []

    if (isRefresh) {
      teamList.value = listData
      page.value = 1
    } else {
      teamList.value = [...teamList.value, ...listData]
    }
...
const goToDetail = (id: number) => {
  uni.navigateTo({ url: `/pages/basic-data/team-detail?id=${id}` })
}

const getStatusText = (status?: string) => {
  const map: Record<string, string> = { ACTIVE: '活跃', INACTIVE: '停用' }
